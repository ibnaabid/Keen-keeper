"use client";
import { NotebookPen, PhoneForwarded, VideoOff } from "lucide-react";
...
const TimelinePage = () => {
  ...
  const allEntries = [
    ...
    (a, b) => new Date(b?.next_due_date) - new Date(a?.next_due_date)
  );
      ...
      {allEntries.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {allEntries.map((entry) => (
            <div
              key={entry.id + entry.type} // 🔥 fix duplicate key
              className={`flex items-center gap-4 p-4 rounded-xl border-l-4 shadow ${getColor(
                entry.type
              )}`}
            >
              {/* Image */}
              <Image
                src={entry?.picture || "https://via.placeholder.com/60"}
                alt={entry?.name || "user"}
                width={60}
                height={60}
